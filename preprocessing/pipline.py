...
class SlidingWindowSegmenter(BaseProcessor):
    ...
    def process(self, data: np.ndarray) -> np.ndarray:
        ...
        n_windows = 1 + (n_times - self.window_len_samples) // self.window_stride_samples
        windows = []
        for win_idx in range(n_windows):
            start = win_idx * self.window_stride_samples
            end = start + self.window_len_samples
            windows.append(data[:, start:end])  # Shape: (n_channels, window_len_samples)
        return np.stack(windows, axis=0)  # Shape: (n_windows, n_channels, window_len_samples)
