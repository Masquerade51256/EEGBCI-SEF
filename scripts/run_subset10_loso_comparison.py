"""
一键运行 XWStroke 10人子集 LOSO 对照实验（快速验证版）

运行两套LOSO实验:
  任务A (左右手): configs/experiment/xwstroke_subset10_loso_lr.yaml
  任务B (患健侧): configs/experiment/xwstroke_subset10_loso_affected.yaml

用法:
    conda activate BCI310
    python scripts/run_subset10_loso_comparison.py
"""
...
def run_exp(config_path, exp_name):
    ...
    if result.returncode != 0:
        ...
        return None
    # Direct path based on fixed experiment name
...
def compare(results_lr, results_aff):
    ...
    print("Saved to results/comparisons/subset10_loso_*.csv/json")

def main():
    print("\n" + "="*60)
    print("XWStroke 10-subject LOSO Quick Validation")
    print("="*60)
    # Use fixed experiment names so directories are deterministic
    res_lr = run_exp("configs/experiment/xwstroke_subset10_loso_lr.yaml", "XWStroke_Subset10_LR")
    res_aff = run_exp("configs/experiment/xwstroke_subset10_loso_affected.yaml", "XWStroke_Subset10_Affected")
